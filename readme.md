# Layout für eine Arbeit an der Hochschule Zittau Görlitz

## Anforderungen
- LaTeX installiert
- optional: VS Code mit LaTeX Setup
## Was wird gemacht?
- Erstellung eines Deckblattes, welches den Anforderungen der HSZG entsprechen __SOLLTE__
- Default layout mit Gliederung, Anhang und Quellen
- Seitennummerierung (römisch für Gliederung, dann arabisch)
- Bereitstellung einer Bibtex-Konfiguration
## How to use
- Diese Repo klonen (nur main branch) und öffnen
- Variablen anpassen: Matrikelnummer, Autor, Modul, Art der Arbeit, etc
- In der Datei .gitignore alle Dateien hinzufügen, welche mit für ein Build benötigt werden
- Zitationen nach bibtex in main.bib speichern und dann im Dokument nutzen (Beispiel kann gelöscht werden)
## Etwas stimmt nicht
Hat sich etwas in den Anforderungen geändert oder entspricht ein Teil überhaupt nicht den Anforderungen?</br>
Fehler können passieren, falls einer gefunden wurde dann bitte per Issue melden.
Contributions sind gerne gesehen.
